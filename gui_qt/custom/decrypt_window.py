...
class CustomCodecDecryptGUI(LibSodiumDecryptGUI):
    ...
    def _create_mode_switch(self, parent, layout):
        ...
        if os.name == 'nt':  # Windows
            ...
            self.custom_codec_radio = QRadioButton("◉ Custom Codec (расшифровка DigitalCodec)")
            self.custom_codec_radio.setChecked(True)  # По умолчанию Custom Codec режим
            self.custom_codec_radio.setToolTip("Расшифровка через DigitalCodec с параметрами M, Q, funType, h1, h2")
        ...
        if is_local_mode:
            ...
            if hasattr(self, 'codec_panel') and self.codec_panel:
                if is_plain_mode:
                    ...
                    # Hybrid или Custom Codec режим - показываем панель кодека (нужна для расшифровки)
    ...
    def _start_encryption(self):
        ...
        if os.name == 'nt' or is_plain_mode or is_hybrid_mode or is_custom_codec_mode:
            ...
            if is_plain_mode:
                ...
                exe_path = FILE_DECODE_PLAIN
                if os.name == 'nt' and not os.path.isfile(exe_path):
                    exe_path = FILE_DECODE_PLAIN_RELEASE
                ...
                # Custom Codec режим (с расшифровкой, но без plain фрагментации)
                ...
                exe_path = FILE_DECODE
                if os.name == 'nt' and not os.path.isfile(exe_path):
                    exe_path = FILE_DECODE_RELEASE
                ...
                # Проверяем наличие исполняемого файла
                exe_path = FILE_DECODE_HYBRID
                if os.name == 'nt' and not os.path.isfile(exe_path):
                    exe_path = FILE_DECODE_HYBRID_RELEASE
