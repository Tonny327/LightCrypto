...
Выберите радиокнопку: **💬 Текстовые сообщения (--msg)**

#### Работа с сообщениями

**Компьютер A (Отправитель):**

После запуска появится поле ввода внизу терминала:
```
┌─ Терминал ───────────────────────┐
│ 🚀 Запуск шифрования...          │
│ ✅ Процесс запущен               │
...
- ✅ **Поле ввода появляется автоматически** в режиме сообщений

---

### 📁 Режим передачи файлов (--file)

**Новая функция!** Безопасная передача файлов любого размера с автоматической фрагментацией и проверкой целостности.

#### Преимущества встроенной передачи файлов

- ✅ **Не требует TAP-интерфейсов** — работает напрямую через UDP
- ✅ **Автоматическая фрагментация** — любой размер файла
- ✅ **Проверка целостности** — CRC32 для чанков, SHA-256 для файла
- ✅ **Повтор потерянных пакетов** — надежная передача
- ✅ **Прогресс в реальном времени** — видно сколько передано
- ✅ **Оригинальное имя** — файл сохраняется с исходным именем

#### Компьютер B (Получатель) — Запуск первым!

1. Выберите режим: **📁 Прием файлов (--file)**

2. (Опционально) Укажите путь сохранения:
   ```
   ┌─ Режим работы ──────────────────────┐
   │ ◉ 📁 Прием файлов (--file)          │
   └──────────────────────────────────────┘
   ...
   ┌─ Путь для сохранения ───────────────┐
   │ [/home/user/received.png] [📁 Выбрать]│
   │ (Пусто = оригинальное имя)          │
   └──────────────────────────────────────┘
   ```

3. Настройте порт (должен совпадать с отправителем!)

4. Нажмите **"▶️ ЗАПУСТИТЬ ШИФРОВАНИЕ"**

**Вывод в терминале:**
```
🚀 Запуск: sudo ./build/tap_decrypt --file --output received.png 12345
📡 Режим приема файла активен
🌐 Ожидаем файл на порту: 12345

📥 Получен заголовок файла: sky.png (245760 байт)
📊 Прогресс: [=====>          ] 45%
📊 Прогресс: [============>   ] 89%
✅ Все чанки получены, сохраняем файл...
✅ Файл успешно сохранен: ./received.png
...
1. Выберите режим: **📁 Отправка файлов (--file)**

2. Выберите файл для отправки:
   ```
   ┌─ Режим работы ──────────────────────┐
   │ ◉ 📁 Отправка файлов (--file)       │
   └──────────────────────────────────────┘
   
   ┌─ Файл для отправки ─────────────────┐
   │ [/home/user/sky.png] [📁 Выбрать]   │
   └──────────────────────────────────────┘
   ```

   Кнопка **"📁 Выбрать"** откроет диалог выбора файла с фильтрами:
   - Все файлы
   - Изображения (png, jpg, jpeg, gif, bmp)
   - Документы (pdf, doc, docx, txt)
   - Архивы (zip, rar, 7z, tar, gz)

3. Введите IP получателя и порт

4. Нажмите **"▶️ ЗАПУСТИТЬ ШИФРОВАНИЕ"**

**Вывод в терминале:**
```
🚀 Запуск: sudo ./build/tap_encrypt --file sky.png 192.168.1.100 12345
📤 Режим отправки файла активен
📁 Файл: sky.png (245760 байт)
🔐 SHA-256: 8a3d5e9f...

📤 Отправлен заголовок файла
📊 Отправка чанков: [=====>     ] 50%
📊 Отправка чанков: [===========>] 100%
...
**Поддерживаются абсолютно любые файлы:**
- 🖼️ Изображения: PNG, JPG, GIF, BMP, TIFF
- 📄 Документы: PDF, DOC, DOCX, TXT, ODT
- 🗜️ Архивы: ZIP, RAR, 7Z, TAR, GZ
- 🎵 Медиа: MP3, MP4, AVI, MKV
- 💾 Бинарные: EXE, DLL, ISO
- 📦 Любые другие форматы

**Размер файла:**
- Нет ограничений (автоматическая фрагментация)
- Тестировалось с файлами до 100 МБ
...
|-------|----------|--------------|
| **LibSodium** | ~110 КБ/с | Средние файлы (до 50 МБ) |
| **Custom Codec** | ~66 КБ/с | Небольшие файлы (до 10 МБ) |

**Примеры:**
- Изображение 2 МБ (LibSodium): ~18 секунд
- Документ PDF 500 КБ: ~5 секунд
- Архив 10 МБ: ~90 секунд
...
**Двойная проверка:**
1. **CRC32** — для каждого чанка (обнаружение повреждений)
2. **SHA-256** — для всего файла (гарантия идентичности)

**При успешной передаче:**
```
✅ Проверка целостности пройдена!
   Отправлен: SHA-256: 8a3d5e9f...
   Получен:   SHA-256: 8a3d5e9f...
   ✅ Хеши совпадают — файл идентичен оригиналу
```
...
**✅ Преимущества режима --file:**
- Не нужно создавать TAP-интерфейсы
- Не нужно настраивать SSH/SCP
- Автоматическая фрагментация больших файлов
- Встроенная проверка целостности
- Работает через любую сеть (LAN/Internet)

**⚠️ Ограничения:**
- Передается только один файл за раз
- Скорость ниже чем у SCP через TAP
- Нет шифрования метаданных (видно имя файла и размер)

**💡 Когда использовать:**
- Для быстрой передачи одного файла
- Когда не хочется настраивать TAP
- Для передачи через сложные сети
- Для тестирования шифрования

**💡 Когда использовать TAP + SCP:**
- Для передачи множества файлов
- Для максимальной скорости
- Для постоянного защищенного канала
