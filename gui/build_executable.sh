#!/bin/bash
# Скрипт для сборки исполняемого файла LightCrypto GUI (tkinter версия)
...
echo -e "${GREEN}╔═══════════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║     Сборка LightCrypto GUI (tkinter)             ║${NC}"
echo -e "${GREEN}╚═══════════════════════════════════════════════════╝${NC}"
echo ""
...
if ! python -c "import PyInstaller" 2>/dev/null; then
    ...
    pip install -r "$GUI_DIR/requirements.txt" --quiet
...
rm -rf "$DIST_DIR" "$GUI_DIR/build" "$GUI_DIR/__pycache__"
find "$GUI_DIR" -type d -name "__pycache__" -exec rm -rf {} + 2>/dev/null || true
