# 🔐 LightCrypto
  ...
  Полученный ключ используется в алгоритме AEAD ChaCha20-Poly1305 для шифрования и аутентификации Ethernet-кадров. (Подробное описание проекта находиться в файле report.docx)
  
- [Структура проекта](#-структура-проекта)
- [Как запустить](#-как-запустить)
- [Графический интерфейс (PyQt6)](#️-графический-интерфейс-pyqt6)
...
gui/                    // Графический интерфейс на Tkinter (старая версия)
gui_qt/                 // Графический интерфейс на PyQt6 (новая версия)
├── main.py             // Точка входа GUI
├── launcher.py         // Стартовое окно
├── role_selector.py    // Выбор роли
├── common/             // Общие компоненты
├── libsodium/          // Окна LibSodium
└── custom/             // Окна Custom Codec
...
- **Python зависимости** (для GUI): `python3-pyqt6` (устанавливается через `apt install python3-pyqt6`)
...
```bash
sudo apt update
sudo apt install -y libsodium-dev iproute2 tcpdump iputils-ping iperf hping3 python3-tk python3-pip python3-dev python3-pyqt6
```
...
Проект включает современный графический интерфейс на PyQt6 с темной темой для удобного управления шифрованием.

### 📦 Установка PyQt6

Для работы графического интерфейса необходимо установить PyQt6:
...
После установки PyQt6 запустите графический интерфейс:
...
- **Выбор типа шифрования**: LibSodium (ChaCha20-Poly1305) или Custom Digital Codec
- **Управление TAP-интерфейсами**: создание и настройка виртуальных сетевых интерфейсов
- **Режимы работы**:
  - 🔀 Ethernet кадры (TAP)
  - 💬 Текстовые сообщения (`--msg`)
  - 📁 Отправка/прием файлов (`--file`)
- **Параметры Custom Codec**: настройка M, Q, типов функций, начальных состояний
- **Тестирование и отладка**: режим отладки и искусственное внесение ошибок
...
- Подсказки и справка по параметрам
