...
```bash
sudo ip netns exec ns2 ~/LightCrypto/build/tap_decrypt 192.168.1.2 12345
```

**Терминал 2 (отправитель в ns1):**

```bash
sudo ip netns exec ns1 ~/LightCrypto/build/tap_encrypt 192.168.1.2 12345
```

### 3. Генерация трафика с помощью ping
...
#### Терминал 1 - приёмник в ns2:
```bash
sudo ip netns exec ns2 ~/LightCrypto/build/tap_decrypt --msg 192.168.1.2 12345
```

#### Терминал 2 - Отправитель в ns1:
```bash
sudo ip netns exec ns1 ~/LightCrypto/build/tap_encrypt --msg 192.168.1.2 12345
```
